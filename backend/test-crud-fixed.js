...
async function testCRUDOperations() {
  try {
    ...
    // First create a user for the car owner
    const carOwner = new User({ ...testUser, email: 'carowner@example.com' });
    await carOwner.save();
    ...
    const allCars = await Car.find({}).select('name brand model pricePerDay availability category');
    ...
    const bookingCar = new Car({
      ...testCar,
      name: 'Booking Car',
      owner: bookingUser._id
    });
    ...
    const reviewCar = new Car({
      ...testCar,
      name: 'Review Car',
      owner: reviewUser._id
    });
    ...
    await User.findByIdAndDelete(carOwner._id);
